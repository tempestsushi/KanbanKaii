...
export function KanbanKaiiPipelineMockup() {
  return (
    <section
      className="relative isolate overflow-hidden rounded-[2rem] border border-[#1a1a24] bg-[#0a0a0c] p-4 text-white shadow-[0_30px_100px_-50px_rgba(124,58,237,0.65)] sm:p-6 lg:p-8"
      aria-label="KanbanKaii app workflow animation mockup"
    >
      <div
        className="absolute -left-24 top-10 h-72 w-72 rounded-full bg-violet-600/24 blur-[90px]"
        aria-hidden="true"
      />
      <div
        className="absolute right-[-8rem] top-1/4 h-80 w-80 rounded-full bg-cyan-400/12 blur-[100px]"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-[-8rem] left-1/2 h-80 w-80 -translate-x-1/2 rounded-full bg-emerald-400/10 blur-[100px]"
        aria-hidden="true"
      />
      <div
      ...
      <div className="relative mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 max-w-xl text-3xl font-black leading-none tracking-[-0.055em] text-white sm:text-4xl">
            Watch a message become a ticket.
          </h3>
        </div>
        <span className="w-fit rounded-full border border-emerald-300/25 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-200">
          Live automation
        </span>
      </div>

      <div className="relative grid gap-4 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="rounded-[1.5rem] border border-[#1a1a24] bg-white/[0.025] p-4 backdrop-blur-xl sm:p-5">
          ...
          <div className="space-y-3">
            {appFlowSteps.map((step, index) => (
              <div
                key={step.title}
                className={`kanban-flow-step kanban-flow-step-${index + 1} relative overflow-hidden rounded-2xl border border-white/10 bg-[#101017]/88 p-4 shadow-2xl shadow-black/20`}
              >
                <div
        ...
        <div className="overflow-hidden rounded-[1.5rem] border border-[#1a1a24] bg-[#0d0d13]/88 shadow-2xl shadow-violet-950/30 backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-[#1a1a24] bg-white/[0.025] px-4 py-3">
...
function SignInScreen() {
  return (
    <div className="kanban-walkthrough-screen kanban-walkthrough-screen-1 absolute inset-4 grid place-items-center">
      <div className="w-full max-w-md rounded-[1.7rem] border border-violet-300/20 bg-[#11111a] p-5 shadow-2xl shadow-violet-950/30">
...
function SlackConnectScreen() {
  return (
    <div className="kanban-walkthrough-screen kanban-walkthrough-screen-2 absolute inset-4 grid place-items-center">
      <div className="w-full max-w-lg rounded-[1.7rem] border border-cyan-300/20 bg-[#11111a] p-5 shadow-2xl shadow-cyan-950/20">
        <div className="mb-6 flex items-center justify-between">
          <div>
          ...
          <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-[10px] font-black text-emerald-200">
            READY
          </span>
        </div>
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="flex items-center gap-4">
            <span className="grid h-14 w-14 place-items-center rounded-2xl bg-white text-2xl font-black text-[#4a154b]">
              #
            </span>
            <div>
              <p className="text-lg font-black text-white">Slack</p>
              <p className="text-xs text-slate-500">Product Team workspace</p>
            </div>
          </div>
          <button
            className="kanban-click-bounce mt-5 w-full rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950"
            type="button"
          >
            Connect Slack
          </button>
        </div>
        <div className="mt-4 rounded-xl border border-emerald-300/20 bg-emerald-400/10 px-4 py-3 text-xs font-bold text-emerald-100">
...
function SlackMessageScreen() {
  return (
    <div className="kanban-walkthrough-screen kanban-walkthrough-screen-3 absolute inset-4 grid place-items-center">
      <div className="w-full max-w-xl rounded-[1.7rem] border border-amber-300/20 bg-[#11111a] p-5 shadow-2xl shadow-amber-950/20">
        <div className="mb-5 flex items-center justify-between">
          <div>
          ...
          <span className="inline-flex items-center gap-2 rounded-full bg-emerald-400/10 px-3 py-1 text-[10px] font-black text-emerald-200">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-300" />
            live
          </span>
        </div>
        <div className="kanban-message-bounce rounded-[1.5rem] bg-[#241f31] p-5 shadow-2xl shadow-black/30">
          <div className="mb-4 flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-300 text-sm font-black text-amber-950">
              AK
            </span>
            <div>
              <p className="text-sm font-black text-white">Aisha</p>
              <p className="text-xs text-slate-500">#product-team · now</p>
            </div>
          </div>
          <p className="text-xl font-bold leading-8 text-slate-100">
            <span className="rounded-lg bg-violet-500/35 px-2 py-1 text-violet-100">
              @Umer
            </span>{" "}
            can you fix the checkout validation before tomorrow?
          </p>
        </div>
        <div className="mt-4 rounded-xl border border-violet-300/20 bg-violet-400/10 px-4 py-3 text-xs font-bold text-violet-100">
...
function TicketCreatedScreen() {
  return (
    <div className="kanban-walkthrough-screen kanban-walkthrough-screen-4 absolute inset-4">
      <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.7rem] bg-[#f7f7fb] p-4 text-slate-950 shadow-2xl shadow-black/30">
