...
drop function if exists public.create_organization_ticket(
...
begin
    ...
    if p_board_id is null then
        if coalesce(v_caller_role, '') not in ('OWNER', 'TEAM_LEAD') then
            raise exception 'Only an organization manager or team lead can assign formal tickets'
                using errcode = '42501';
        end if;
    elsif not public.can_manage_organization_board(p_organization_id, p_board_id) then
        raise exception 'Only an organization manager or board manager can assign project-board tickets'
            using errcode = '42501';
    end if;
    ...
    if p_board_id is not null and not exists (
        select 1
        from public.organization_board_members board_member
        where board_member.organization_id = p_organization_id
          and board_member.board_id = p_board_id
          and board_member.user_id = p_assignee_user_id
    ) then
        raise exception 'Project-board tickets can only be assigned to board members'
            using errcode = '22023';
    end if;
